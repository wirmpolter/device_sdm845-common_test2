...
public class DozeSettings extends PreferenceActivity implements OnPreferenceChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        boolean dozeEnabled = DozeUtils.isDozeEnabled(mContext);
        ...
        mAmbientDisplayPreference =
            (SwitchPreference) findPreference(DozeUtils.AMBIENT_DISPLAY_KEY);
        // Read from DOZE_ENABLED secure setting
        mAmbientDisplayPreference.setChecked(dozeEnabled);
        mAmbientDisplayPreference.setOnPreferenceChangeListener(this);

        mPickUpPreference =
            (SwitchPreference) findPreference(DozeUtils.PICK_UP_KEY);
        mPickUpPreference.setOnPreferenceChangeListener(this);

        mHandwavePreference =
            (SwitchPreference) findPreference(DozeUtils.GESTURE_HAND_WAVE_KEY);
        mHandwavePreference.setOnPreferenceChangeListener(this);

        mPocketPreference =
            (SwitchPreference) findPreference(DozeUtils.GESTURE_POCKET_KEY);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (DozeUtils.AMBIENT_DISPLAY_KEY.equals(key)) {
            mAmbientDisplayPreference.setChecked(value);
            DozeUtils.enableDoze(value, mContext);
            return true;
        } else if (DozeUtils.PICK_UP_KEY.equals(key)) {
            mPickUpPreference.setChecked(value);
            DozeUtils.startService(mContext);
            return true;
        } else if (DozeUtils.GESTURE_HAND_WAVE_KEY.equals(key)) {
            mHandwavePreference.setChecked(value);
            DozeUtils.startService(mContext);
            return true;
        } else if (DozeUtils.GESTURE_POCKET_KEY.equals(key)) {
